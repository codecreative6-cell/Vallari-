...
/* ──────────────────────────
   MARQUEE
────────────────────────── */
const words = ['Happy Birthday','·','Vallari Batra','·','Birthday Girl','·','deluluu','·','vallariiiii._','·','🌸','·','forever young','·','cosmic queen','·','main character','·'];
...
const photoSrcs = [];
const galleryWords = ['gorgeous ✨','stunning 🤍','ethereal 🌸','magic 🦋','radiant 💫','iconic 🎀','beautiful 🌷','angelic 🕊️','flawless 👑','dreamy ☁️','perfect 💎','sweet 🍓'];

for(let i=1;i<=totalPhotos;i++){
  const src=`img/img (${i}).webp`;
  photoSrcs.push(src);
  ...
  const img=document.createElement('img'); img.src=src; img.alt=`Vallari — memory ${i}`; img.loading='lazy';
...
if (bgAudio) {
  ...
  const playAudio = () => {
    bgAudio.play().then(() => {
      ['click', 'touchstart', 'scroll'].forEach(evt => document.removeEventListener(evt, playAudio));
    }).catch(e => console.log('Audio play prevented', e));
  };
  ['click', 'touchstart', 'scroll'].forEach(evt => document.addEventListener(evt, playAudio));
}
